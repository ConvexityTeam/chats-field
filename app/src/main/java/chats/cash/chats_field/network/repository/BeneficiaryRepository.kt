...
class BeneficiaryRepository(
    ...
    override suspend fun getAllCampaigns(): Flow<NetworkResponse<List<ModelCampaign>>>{
        return dataSource.getAllCampaigns(preferenceUtil.getNGOId(),
            preferenceUtil.getNGOToken()).onEach {
    ...
    override suspend fun getAllCampaignForms(): Flow<NetworkResponse<List<CampaignForm>>> {
        ...
        return dataSource.getAllCampaignForms(preferenceUtil.getNGOId(),preferenceUtil.getNGOToken()).onEach {
            if(it is NetworkResponse.Success){
    ...
    override suspend fun OnboardBeneficiary(beneficiary: Beneficiary,
        ...
        return if(isOnline) dataSource.OnboardBeneficiary(beneficiary, true,preferenceUtil.getNGOId(),
            preferenceUtil.getNGOToken()) else flow{
    ...
    override suspend fun OnboardVendor(
        ...
        return if(isOnline) dataSource.OnboardVendor(beneficiary, true,
            preferenceUtil.getNGOId(),preferenceUtil.getNGOToken()) else flow{
            emit(NetworkResponse.Offline())
        }
    }
