...
class OrganizationBeneficiaryRepositoryTest {
    ...
    fun getAllCampaigns() = runTest {
        ...
        assert(
            campaigns.first() is NetworkResponse.Success && (
                campaigns.first()
    ...
    fun getCampaignSurvey() = runTest {
        val expectedCampaignSurvey = CampaignSurveyResponse.CampaignSurveyResponseData(
            1,
            2,
            "",
            1,
        ...
        assert(
            campaignSurvey.first() is NetworkResponse.Success && (
                campaignSurvey.first()
                    as NetworkResponse.Success
                ).body == expectedCampaignSurvey,
        )
    }

    @Test
    fun getAllCampaignForms() = runTest {
        val expectedAllCampaign = listOf(
            ModelCampaign(
        ...
        val expectedAllCampaignForms = listOf(
            CampaignForm(
                1,
                expectedAllCampaign,
                "2sec",
        ...
        assert(
            allCampaignsForm.first() is NetworkResponse.Success && (
                allCampaignsForm.first()
                    as NetworkResponse.Success
                ).body == expectedAllCampaignForms,
        )
    }
}
